...
base.SCENARIO_ROOT = ROOT / "data" / "Kurse" / "LF04-Scenarien"
...
base.QUIZ_DB_PATH = ROOT / "data" / "Kurse" / "LF04-Quiz.db"
base.BASE_QUESTION_COUNT = 60
base.PRE_STAGE_QUESTION_COUNT = 120
base.BATCH_SIZE = 100
base.DEFAULT_QUESTION_LIMIT = 100
base.MAX_STAGE_COUNT = 9
base.MAX_QUESTION_LIMIT = base.PRE_STAGE_QUESTION_COUNT + (base.MAX_STAGE_COUNT * base.BATCH_SIZE)
base.SHORT_TEXT_DISTRACTORS = (
    (
        "Sie bleibt bei Sicherheit nur allgemein und verknuepft Schutzbedarf, Rolle und Massnahme nicht klar.",
        "Bei LF04 muessen Schutzbedarf, Rolle und passende Massnahme nachvollziehbar verbunden werden und duerfen nicht allgemein bleiben.",
    ),
    (
        "Sie nennt einzelne Risiken, ohne Standards, organisatorische Verantwortung und konkrete Schutzwirkung sauber zusammenzufuehren.",
        "Eine tragfaehige LF04-Antwort fuehrt Risiken, Standards, Verantwortungen und Schutzwirkung nachvollziehbar zusammen.",
    ),
    (
        "Sie betrachtet fast nur Einzelgeraete und blendet Prozess, Dokumentation und Sicherheitsregeln weitgehend aus.",
        "LF04 fragt nach wirksamen Sicherheitsloesungen mit Prozess-, Dokumentations- und Regelbezug und nicht nur nach Einzelgeraeten.",
    ),
)
...
def normalize_pool_slug(value: str) -> str:
    ...
    return slug or "lf04-pool"


def build_concept_key(pool_slug: str, title: str) -> str:
    return f"lf04::{pool_slug}::{base.slugify_visible_text(title)}"
...
def load_course_description() -> str:
    ...
    if intro:
        return f"Quizdatenbank fuer LF04. {intro}"
    return "Quizdatenbank fuer LF04."
...
def build_pre_stage_pools() -> list[dict[str, Any]]:
    ...
    if total_questions != base.PRE_STAGE_QUESTION_COUNT:
        raise ValueError(
            f"LF04-Vorstufe erzeugt {total_questions} Fragen statt {base.PRE_STAGE_QUESTION_COUNT}."
        )
    ...
    if base_only != base.BASE_QUESTION_COUNT:
        raise ValueError(
            f"LF04-Basis erzeugt {base_only} Fragen statt {base.BASE_QUESTION_COUNT}."
        )
...
def build_base_question(scenario_rel_path: str, pool_slug: str, raw_question: dict[str, Any]) -> dict[str, Any]:
    ...
    if question_type in {"single_choice", "multi_select"}:
        if title and title not in prompt and prompt.startswith(
            (
                "Welche Option",
                "Welche Aussage",
                "Welche Szenarien",
...
_original_rebuild_database = base.rebuild_database


def rebuild_database(pools: list[dict[str, Any]]) -> tuple[int, int, int, int]:
    result = _original_rebuild_database(pools)
