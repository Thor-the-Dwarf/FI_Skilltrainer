...
STAGE_REPEAT_TRAILERS: tuple[str, ...] = (
    "unter besonderer Beachtung der praktischen Absicherung",
    "mit Blick auf Dokumentation und Nachvollziehbarkeit",
    "unter Einbezug von Test und Betriebsreife",
    "mit Fokus auf Risiken und Fehlerfaelle",
    "unter Beruecksichtigung der Abnahme",
    "mit Augenmerk auf Wartbarkeit und Uebergabe",
    "unter Einbezug von Schnittstellen und Rueckmeldungen",
    "mit Blick auf Schutzbedarf und Folgeschritte",
)
...
STAGE_VARIANT_CONFIGS = (
    {
        ...
        "single_templates": (
            "Welche Entscheidung passt fachlich am besten?",
            "Welche Antwort trägt die Aufgabe fachlich am besten?",
            "Was trifft hier am besten zu?",
        ),
        "multi_templates": (
            "Welche Punkte passen fachlich dazu?",
            "Welche Aussagen tragen die Aufgabe fachlich?",
            "Was gehört hier fachlich dazu?",
        ),
    },
    {
        "stage_label": "stage2",
        ...
        "single_templates": (
            "Welche Lösung passt im beschriebenen Praxisfall?",
            "Welche Reaktion ist hier tragfähig?",
            "Was ist in dieser Situation der passende Schritt?",
        ),
        "multi_templates": (
            "Welche Punkte tragen den Praxisfall fachlich?",
            "Welche Aussagen sind hier praxisnah richtig?",
            "Was gehört in diesem Fall fachlich dazu?",
        ),
    },
    {
        "stage_label": "stage3",
        ...
        "single_templates": (
            "Wie ist die Situation fachlich einzuordnen?",
            "Welche Antwort ordnet den Vorgang richtig ein?",
            "Welche Auswahl passt zur fachlichen Einordnung?",
        ),
        "multi_templates": (
            "Welche Punkte ordnen die Situation richtig ein?",
            "Welche Aussagen passen zur fachlichen Einordnung?",
            "Was ist hier fachlich einzuordnen?",
        ),
    },
    {
        "stage_label": "stage4",
        ...
        "single_templates": (
            "Welche Antwort steuert den Vorgang in die richtige Richtung?",
            "Welche Entscheidung hält die Bearbeitung fachlich auf Kurs?",
            "Welcher Schritt steuert die Bearbeitung sinnvoll weiter?",
        ),
        "multi_templates": (
            "Welche Punkte halten die Bearbeitung auf Kurs?",
            "Welche Aussagen tragen zur fachlichen Steuerung bei?",
            "Was ist für die Steuerung des Vorgangs wichtig?",
        ),
    },
    {
        "stage_label": "stage5",
...
def pick_template(source_key: str, templates: tuple[str, ...], *, offset: int = 0) -> str:
    if not templates:
        raise ValueError("Leere Template-Liste.")
    base_index = sum(ord(char) for char in source_key) % len(templates)
    return templates[(base_index + offset) % len(templates)]


def apply_stage_repeat_trailer(prompt: str, pass_index: int, template_count: int) -> str:
    if pass_index < template_count:
        return prompt

    trailer = STAGE_REPEAT_TRAILERS[(pass_index - template_count) % len(STAGE_REPEAT_TRAILERS)]
    if prompt.endswith("?"):
        return f"{prompt[:-1]} {trailer}?"
    if prompt.endswith("."):
        return f"{prompt[:-1]} {trailer}."
    return f"{prompt} {trailer}"
...
def clone_choice_stage_variant(
    ...
    *,
    ...
    prompt = normalize_visible_text(
        pick_template(
            f"{question['source_ref']}::{stage_suffix}",
            templates,
            offset=pass_index,
        ).format(title=title)
    )
    cloned["prompt"] = apply_stage_repeat_trailer(prompt, pass_index, len(templates))
