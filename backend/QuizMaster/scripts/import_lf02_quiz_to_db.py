...
base.SCENARIO_ROOT = ROOT / "data" / "Kurse" / "LF02-Scenarien"
...
base.QUIZ_DB_PATH = ROOT / "data" / "Kurse" / "LF02-Quiz.db"
base.BASE_QUESTION_COUNT = 59
base.PRE_STAGE_QUESTION_COUNT = 118
base.BATCH_SIZE = 100
...
base.SHORT_TEXT_DISTRACTORS = (
    (
        "Sie bleibt absichtlich allgemein und vermeidet klare technische Aussagen.",
        "Eine belastbare Antwort braucht bei LF02 klare technische Aussagen und bleibt nicht bewusst vage.",
    ),
    (
        "Sie sammelt lose Stichworte, ohne Bedarf, Begründung und Umsetzung erkennbar zu verbinden.",
        "Stichworte allein reichen nicht; bei LF02 müssen Bedarf, Begründung und Umsetzung nachvollziehbar verbunden sein.",
    ),
    (
        "Sie konzentriert sich fast nur auf persönliche Vorlieben statt auf Arbeitsplatz, Nutzung und Betrieb.",
        "LF02 fragt nach tragfähigen Lösungen für Arbeitsplatz und Betrieb und nicht nach bloßen Vorlieben.",
    ),
)
...
def normalize_pool_slug(value: str) -> str:
    ...
    return slug or "lf02-pool"


def build_concept_key(pool_slug: str, title: str) -> str:
    return f"lf02::{pool_slug}::{base.slugify_visible_text(title)}"
...
def load_course_description() -> str:
    ...
    if intro:
        return f"Quizdatenbank für LF02. {intro}"
    return "Quizdatenbank für LF02."
...
def build_pre_stage_pools() -> list[dict[str, Any]]:
    ...
    if total_questions != base.PRE_STAGE_QUESTION_COUNT:
        raise ValueError(
            f"LF02-Vorstufe erzeugt {total_questions} Fragen statt {base.PRE_STAGE_QUESTION_COUNT}."
        )
    ...
    if base_only != base.BASE_QUESTION_COUNT:
        raise ValueError(
            f"LF02-Basis erzeugt {base_only} Fragen statt {base.BASE_QUESTION_COUNT}."
        )
...
def build_base_question(scenario_rel_path: str, pool_slug: str, raw_question: dict[str, Any]) -> dict[str, Any]:
    ...
    if question_type == "ordering":
        if prompt == "Bringen Sie die Schritte in eine sinnvolle Reihenfolge.":
            prompt = f"Bringen Sie die Schritte bei {title} in eine sinnvolle Reihenfolge."
