...
base.SCENARIO_ROOT = ROOT / "data" / "Kurse" / "LF03-Scenarien"
...
base.QUIZ_DB_PATH = ROOT / "data" / "Kurse" / "LF03-Quiz.db"
...
base.SHORT_TEXT_DISTRACTORS = (
    (
        "Sie bleibt absichtlich allgemein und vermeidet klare Netz- und Betriebsbezüge.",
        "Eine belastbare Antwort braucht bei LF03 klare Netz- und Betriebsbezüge und bleibt nicht bewusst vage.",
    ),
    (
        "Sie sammelt lose Stichworte, ohne Status, Prüfung und Restrisiko erkennbar zu verbinden.",
        "Stichworte allein reichen nicht; bei LF03 müssen Status, Prüfung und Restrisiko nachvollziehbar verbunden sein.",
    ),
    (
        "Sie konzentriert sich fast nur auf Einzelgeräte statt auf Netzbereich, Dienste und Betrieb.",
        "LF03 fragt nach tragfähigen Lösungen für Client-Einbindung, Dienste und Betrieb und nicht nur nach Einzelgeräten.",
    ),
)
...
def normalize_pool_slug(value: str) -> str:
    ...
    return slug or "lf03-pool"


def build_concept_key(pool_slug: str, title: str) -> str:
    return f"lf03::{pool_slug}::{base.slugify_visible_text(title)}"
...
def load_course_description() -> str:
    ...
    if intro:
        return f"Quizdatenbank für LF03. {intro}"
    return "Quizdatenbank für LF03."
...
def build_pre_stage_pools() -> list[dict[str, Any]]:
    ...
    if total_questions != base.PRE_STAGE_QUESTION_COUNT:
        raise ValueError(
            f"LF03-Vorstufe erzeugt {total_questions} Fragen statt {base.PRE_STAGE_QUESTION_COUNT}."
        )
    ...
    if base_only != base.BASE_QUESTION_COUNT:
        raise ValueError(
            f"LF03-Basis erzeugt {base_only} Fragen statt {base.BASE_QUESTION_COUNT}."
        )
